  ...
  return (
    <SafeScreen>
      ...
      >
        <View className='px-6 pb-4'>
          <View className='bg-surface rounded-3xl p-6'>
            <View className='flex-row items-center'>
              <View className='reltive '>
                <Image
                source={user?.imageUrl}
                style={{ width: 80, height: 80, borderRadius: 40 }}
                transition={200}
                />
                <View className='absolute bottom-1 right-1 rounded-full size-7 bg-primary items-center justify-center border-2 border-surface '>
        ...
        <View className='flex-row flex-wrap mx-6 mb-3'>
          {MENU_ITEMS.map((item)=>(
            <TouchableOpacity
            key={item.id}
        ...
        <View className='mb-4 mx-6 bg-surface rounded-2xl p-4'>
          ...
          >
            <View className='flex-row items-center'>
              ...
              <Text
              className='text-text-primary font-semibold ml-1'
              >
                Notifications
              </Text>
